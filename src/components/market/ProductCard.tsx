...
export const ProductCard: React.FC<ProductCardProps> = memo(({
  ...
  return (
    ...
    >
      <div className={`relative ${horizontal ? 'aspect-square' : 'aspect-[4/3]'} bg-gradient-to-br from-emerald-50 to-teal-50 dark:from-emerald-900/20 dark:to-teal-900/20 overflow-hidden`}>
        ...
        {isLoggedIn && (
          <button
            ...
            disabled={favLoading}
            className="absolute top-2 left-2 z-10 w-8 h-8 bg-[var(--color-surface)]/90 backdrop-blur-sm rounded-full flex items-center justify-center shadow-sm transition-colors hover:bg-[var(--color-surface)]"
          >
            {favLoading ? (
      ...
      <div className="p-5 flex flex-col flex-1">
        {storeInfo}
        <p className="text-[15px] font-semibold text-gray-800 dark:text-gray-100 line-clamp-1 mb-2">{product.name}</p>
        <p className="text-[10px] text-gray-400 dark:text-gray-500 mt-1">{product.category}</p>
        <p className="text-xl font-bold text-emerald-600 dark:text-emerald-400 mb-3">
